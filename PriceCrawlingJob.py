...
class PriceCrawlingJob ():
    basic_info_url_format = 'https://tw.finance.yahoo.com/d/s/company_%s.html'
    price_url_format = 'https://tw.finance.yahoo.com/q/q?s=%s' 
    ...
    def do_fetch(self, url):
        ...
        # Read the web content (Big5 encoded) in and convert it to unicode
        return unicode(content.read(), 'Big5', 'ignore')

    def parse(self):
        self.data['id'] = str(self.target_id)

        # Parse for ROE
        mo = re.search(unicode('<td .*?>股東權益報酬率</td>\\s*<td .*?>([0-9\\.\\-]+)([^<]*)</td>', 'utf-8'), self.basic_info_raw)
        if mo == None:
            raise RuntimeError('PriceCrawlingJob: unable to parse ROE value. stock id: ' + str(self.target_id))
        elif len(mo.groups()) != 2:
            raise RuntimeError('PriceCrawlingJob: unexpected number of matched groups for RE of ROE.')
        if mo.group(1) == '-':
            self.data['roe'] = float(0.0)
        else:
            self.data['roe'] = float(mo.group(1))
    
        # Parse for price
        mo = re.search(unicode(' nowrap><b>([0-9\\.]+)</b>', 'utf-8'), self.price_info_raw)
        if mo == None:
        ...
        self.data['price'] = float(mo.group(1))
